...
class UVCOM(nn.Module):
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, epoch=None, src_aud=None, src_aud_mask=None):
        ...
        # for global token
        mask_ = torch.tensor([[True]], device='cuda').repeat(mask.shape[0], 1)
...
class SetCriterion(nn.Module):
    ...
    def loss_saliency(self, outputs, targets, indices, log=True):
        ...
        batch_indices = torch.arange(len(saliency_scores), device='cuda')
        ...
        for i in batch_indices:
            ...
            for j in range(num_pairs):
                if neg_indices[i, j] == -1:
                    n_score1 = torch.tensor(0, device='cuda')
                else:
    ...
    def get_sim_loss(self,outputs,targets,indices):
        ...
        gt_inter = torch.eye(bs, device='cuda')
        ...
        # loss_clip_intra
        gt_clip_intra = torch.zeros_like(sim_clip_intra, device='cuda')
